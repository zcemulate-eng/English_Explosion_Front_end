            ...
            >
              {sentences.length > 0 && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16, flexWrap: 'wrap' }}>
                  <button
                    type="button"
                    onClick={() => { setPracticeMode('full_text'); setShowComparison(false); setInputText(''); }}
                    style={{
                  ...
                  <button
                    type="button"
                    onClick={() => { setPracticeMode('fill_in_blank'); setShowComparison(false); setInputText(''); }}
                    style={{
                      padding: '6px 18px', borderRadius: 999, fontWeight: 700, cursor: 'pointer',
                      fontSize: 'clamp(0.85rem,1.1vw,1.1rem)',
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 26, marginTop: 18 }}>
                ...
                <button type="button"
                  onClick={() => {
                    if (!currentSentence || practiceMode === 'fill_in_blank') return;
                    const next = hintLevel + 1;
                    setHintLevel(next);
                    if (next === 1) {
                      const firstLetters = currentSentence.content
                        .trim().split(/\s+/)
                        .map((w) => w[0] + '_')
                        .join(' ');
                      setInputText(firstLetters);
                    } else if (next === 2) {
                      const words = currentSentence.content.trim().split(/\s+/);
                      setInputText(words.slice(0, Math.ceil(words.length / 2)).join(' ') + ' ...');
                    } else {
